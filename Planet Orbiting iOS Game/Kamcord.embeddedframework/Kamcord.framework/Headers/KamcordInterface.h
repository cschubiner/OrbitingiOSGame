...
/*
 * Engine Detection for Cocos Integration
 */
#import "Core/OpenGL/KCLoader.h"
...
/*
 * The title of the video on Kamcord watch pages, and on YouTube.
 */
+ (void)setVideoTitle:(NSString *)title;
+ (NSString *)videoTitle;

/*
 * YouTube share defaults
 */
 ...
 */
+ (void)setFacebookDescription:(NSString *)facebookDescription;
+ (NSString *)facebookDescription;

/*
 * Twitter share defaults
 */
...
/*
 * Deprecated sharing methods. Please don't use these if possible.
 * We'll be getting rid of them in 3 months (May 2013).
 */
 ...
 * By default, most of our games run at 60FPS while the recorded video is at 30FPS.
 *
...
/*
 * After stopRecording is called, this returns true if
 * the recorded video actually had > 0 frames. A video
 * May have 0 frames if you call stopRecording immediately
 * after stopRecording, which doesn't give the system enough
 * time to capture even one frame.
 */
+ (BOOL)isVideoValid;
